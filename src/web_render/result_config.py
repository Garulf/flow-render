...
WINDOWS_ICON_SUFFIXES = {'.exe', '.dll', '.icl', '.cpl', '.scr', '.lnk'}
REMOTE_ICON_TIMEOUT_SECONDS = 15
ICON_RENDER_SIZE = 64
...
def get_remote_icon(icon: str) -> str:
    response = httpx.get(icon, timeout=REMOTE_ICON_TIMEOUT_SECONDS)
...
def _hicon_to_png_data(hicon: int, size: int = ICON_RENDER_SIZE) -> Optional[bytes]:
...
def resolve_icon(icon: str, base_path: Optional[str] = None) -> str:
    ...
    if source_path.exists() and source_path.is_file():
        ...
        if suffix in WINDOWS_ICON_SUFFIXES:
