...
def test_detect_canvas_size_returns_none_when_no_marker(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "promo.css").write_text("#WindowBorder { color: red; }\n")

    assert detect_canvas_size(["promo.css"]) is None


def test_detect_canvas_size_returns_none_for_empty_css_files():
    assert detect_canvas_size([]) is None
...
def test_detect_canvas_size_resolves_bundled_static_theme_with_no_marker(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
